  ...
  const handleDeleteAgent = useCallback(
    async (agentId: string) => {
      ...
      const confirmed = window.confirm(
        `Delete ${agent.name}? This removes the agent from gateway config + scheduled automations and moves its workspace/state into the trash directory within OPENCLAW_STATE_DIR on the gateway host.`
      );
      if (!confirmed) return;
